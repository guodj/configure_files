...
oneMinuteSignup.AppService = {
    ...
    getTriggerScript: function (scriptType, data) {
        ...
        function quote(textToQuote){
            return textToQuote.replace(/\"/g,'\\"');
        }

        if (scriptType === oneMinuteSignup.ScriptType.request) {
            return 'var context = new LastPass.Web.Automation.Context(LastPass.Extension.ExtensionHostEnvironment); context.run(function(){requestPasswordReset(context, "' + quote(data) + '");})';
        }
        if (scriptType === oneMinuteSignup.ScriptType.change) {
            return 'var context = new LastPass.Web.Automation.Context(LastPass.Extension.ExtensionHostEnvironment); context.run(function(){passwordReset(context, "' + quote(data) + '");})';
        }
