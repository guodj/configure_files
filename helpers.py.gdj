...
subsol is Modified by Dongjie Guo, UM, on  Sat Sep 24 00:56:15 CST 2016,
in oder to support pd.DatetimeIndex'''
...
def subsol(datetime):
    ...
    Parameters
    ==========
    datetime : pd.DatetimeIndex or pd.Timestamp or string
    ...
    if not ((1601 <= year) & (year<=2100)).all():
    ...
    nleap = nleap - 99
    fp = (year <= 1900)
    ncent = np.floor((year[fp]-1601)/100)
    ncent = 3 - ncent
    nleap[fp] = nleap[fp] + ncent
